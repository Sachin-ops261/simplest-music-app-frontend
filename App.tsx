...
export default function App() {
  ...
  useEffect(() => {
    async function setupPlayer() {
      try {
        ...
        // 🛡️ Fixes 'CAPABILITY_PLAY of null' exception by passing runtime-safe strings directly
  ...
  useEffect(() => {
    const trackChangedListener = TrackPlayer.addEventListener(Event.PlaybackActiveTrackChanged, async (evt) => {
      if (evt.track) {
  ...
  const handleSearch = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`${BACKEND_URL}/api/search/instant?q=${encodeURIComponent(searchQuery)}`);
      const data = await response.json();
  ...
  return (
    <View style={styles.appShell}>
      ...
      <View style={[styles.headerContainer, isSearchFocused && styles.headerContainerFocused]}>
        {!isSearchFocused ? (
        ...
        <View style={[styles.searchBarRow, isSearchFocused && styles.searchBarRowFull]}>
      ...
      {!isSearchFocused ? (
        ...
        <ScrollView contentContainerStyle={styles.mainContainer} showsVerticalScrollIndicator={false}>
          ...
          {trendingTracks.length === 0 && (
            <ActivityIndicator size="small" color="#1DB954" style={{ marginVertical: 30 }} />
          )}

          {trendingTracks.map((song, index) => {
            const isCurrent = currentTrack?.id === song.id;
            return (
              <Pressable key={`trend-${song.id}-${index}`} style={styles.songCard} onPress={() => playSingleTrack(song)}>
                <Image source={{ uri: song.art }} style={styles.songArtThumb} />
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Text style={[styles.songTitleText, isCurrent && { color: '#1DB954' }]} numberOfLines={1}>
                    {song.title}
                  </Text>
                  <Text style={styles.songArtistText} numberOfLines={1}>{song.artist}</Text>
                </View>
                {isCurrent && isPlaying ? <Pause size={18} color="#1DB954" /> : <Play size={18} color="#aaa" />}
              </Pressable>
            );
          })}
        </ScrollView>
        ...
        <ScrollView contentContainerStyle={styles.mainContainer} style={styles.blackFocusContainer}>
          {isLoading ? (
            ...
            <View style={styles.emptySearchCenterBlock}>
              ...
              <Text style={styles.emptySearchText}>Type your favorite song title above and press enter.</Text>
            </View>
          ) : (
            searchResults.map((song, index) => {
              const isCurrent = currentTrack?.id === song.id;
              return (
                <Pressable key={`search-${song.id}-${index}`} style={styles.songCard} onPress={() => playSingleTrack(song)}>
                  <Image source={{ uri: song.art }} style={styles.songArtThumb} />
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={[styles.songTitleText, isCurrent && { color: '#1DB954' }]} numberOfLines={1}>
                      {song.title}
      ...
      {/* MINI CONTROLLER BAR */}
      {currentTrack && (
        <Pressable style={styles.miniPlayer} onPress={() => { setIsPlayerModalOpen(true); setIsQueueVisible(false); }}>
                ...
                <View style={styles.queueContainerView}>
                  ...
                  <ScrollView style={styles.queueScrollView} showsVerticalScrollIndicator={false}>
                    {queueTracks.map((qSong, qIdx) => (
                      <Pressable key={`queue-${qSong.id}-${qIdx}`} style={styles.queueCard} onPress={() => playSingleTrack(qSong)}>
              ...
              <View style={styles.playerControlsDock}>
                <View style={styles.progressContainer}>
                  <View style={styles.progressBarBg}>
                    <View style={[styles.progressBarFill, { width: `${(progress.position / (currentTrack.durationSec || 1)) * 100}%` }]} />
                  </View>
                ...
                <Pressable style={styles.queueToggleArrowButton} onPress={() => setIsQueueVisible(!isQueueVisible)}>
                  {isQueueVisible ? <ChevronDown size={24} color="#aaa" /> : <ChevronUp size={24} color="#aaa" />}
                  <Text style={styles.queueToggleActionText}>{isQueueVisible ? "Hide Queue" : "Swipe / Click to See Queue"}</Text>
                </Pressable>
              </View>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  headerContainer: { paddingHorizontal: 16, paddingTop: 60, backgroundColor: '#0d0d0d', borderBottomWidth: 1, borderBottomColor: '#161616', paddingBottom: 12 },
  ...
  searchBarRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1a1a1a', borderRadius: 22, paddingHorizontal: 16, height: 44, flex: 1 },
  ...
  emptySearchCenterBlock: { flex: 1, alignItems: 'center', justifyContent: 'center', marginTop: 100, paddingHorizontal: 40 },
  emptySearchText: { color: '#444', fontSize: 13, textAlign: 'center', lineHeight: 18 },
  
  songCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#141414', padding: 12, borderRadius: 8, marginBottom: 10 },
  ...
  miniPlayer: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 72, backgroundColor: '#1c1c1c', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, borderTopWidth: 1, borderTopColor: '#282828' },
  miniArt: { width: 44, height: 44, borderRadius: 4 },
  miniTitle: { color: '#fff', fontSize: 14, fontWeight: '500' },
  miniArtist: { color: '#aaa', fontSize: 12 },
  miniControlsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: 110 },
  miniControlBtn: { padding: 6 },
  
  modalContainer: { flex: 1, backgroundColor: '#0d0d0d', paddingTop: 20 },
  modalHeaderRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 4 },
  modalNowPlayingTitle: { color: '#fff', fontSize: 13, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: 1.5 },
  closeButton: { padding: 16 },
  modalContent: { flex: 1, alignItems: 'center', paddingHorizontal: 24, justifyContent: 'space-between' },
  ...
  queueCard: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 8, marginBottom: 8, backgroundColor: '#141414' },
  ...
  controlsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', paddingHorizontal: 8 },
  playButtonBig: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' },
  
  queueToggleArrowButton: { width: '100%', alignItems: 'center', justifyContent: 'center', marginTop: 24, paddingTop: 10 },
  queueToggleActionText: { color: '#444', fontSize: 11, fontWeight: 'bold', marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.5 }
});